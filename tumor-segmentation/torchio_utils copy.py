...
def torchio_compose_train(image_paths, label_paths, control_paths, 
    ...
    if changes == 'medium':
        training_transform = tio.Compose([
            ...
            tio.OneOf({
                tio.RandomAffine(scales=(0.7, 1.3),
                                degrees=(-5, 5)),
                tio.RandomElasticDeformation(max_displacement=(10, 10, 0),
                                             num_control_points=10)},
                p=1),
    ...
    if changes == 'large':
        training_transform = tio.Compose([
            tio.RandomAnisotropy(p=0.25),
            tio.OneOf({
                tio.RandomAffine(scales=(0.5, 1.5),
                                degrees=(-10, 10)),
                tio.RandomElasticDeformation(max_displacement=(15, 15, 0),
...
def torchio_validation_composition(image_paths, base_dir,
                          invert_colors=True,
                          cropsize = (400,991),
                          dataset_ID = None):
